...
# get the html for each of the urls in topic_url and extract all the <a> tags from it
    ...
    def extract_article_links(self, soup):
        ...
        for a in soup.find_all('a'):
            ...
            if href and "https://www.harvardmagazine.com/" in href:    # All article topics have the domain in them. This removes internal references and youtube and social media links
                path_segments = [segment for segment in urlparse(href).path.split('/') if segment]
                if (len(path_segments) == 2         # Article paths need to hae 2 path segments
                    and not path_segments[0].isdigit() # Article Path cannot be just numbers - it is only numbers if the link is referring to magazine issue date
                    and "/topic/" not in href   # Removing all references to topic pages, focusing only on articles: 
